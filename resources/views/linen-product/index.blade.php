...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    <x-crud.breadcrumb :items="[['label' => 'Admin', 'route' => route('admin')],['label' => 'Linen Product']]" />
    <x-ui.card>
        <x-slot:header>
            <x-crud.page-header title="Linen Product" :createRoute="route('linen-products.create')" />
        </x-slot:header>
        ...
        <div class="overflow-x-auto rounded-lg shadow">
            <table class="w-full text-sm text-left border-collapse bg-white dark:bg-gray-800">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($linenProducts as $index => $linenProduct)
                        <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent dark:hover:from-gray-700/50 dark:hover:to-transparent transition-all duration-200">
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-1">
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Linen Type Id:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $linenProduct->linen_type_id ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Name:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $linenProduct->name ?? '-' }}</span>
                                        </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <x-crud.action-buttons :model="$linenProduct" resource="linen-products" />
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p>No Linen Product available</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
