                    ...
                    @forelse($operationLogs as $index => $operationLog)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 text-gray-900 dark:text-gray-100">
                                <strong>Operation Id:</strong> {{ $operationLog->operation_id ?? '-' }}<br><strong>Employee Id:</strong> {{ $operationLog->employee_id ?? '-' }}<br><strong>Action Name:</strong> {{ $operationLog->action_name ?? '-' }}<br><strong>Old Values:</strong> {{ $operationLog->old_values ?? '-' }}<br><strong>New Values:</strong> {{ $operationLog->new_values ?? '-' }}
                            </td>
                            <td class="px-6 py-4">
