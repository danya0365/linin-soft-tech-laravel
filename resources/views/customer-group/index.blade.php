...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    <x-crud.breadcrumb :items="[['label' => 'Admin', 'route' => route('admin')],['label' => 'Customer Group']]" />
    <x-ui.card>
        <x-slot:header>
            <x-crud.page-header title="Customer Group" :createRoute="route('customer-groups.create')" />
        </x-slot:header>
        ...
        <div class="overflow-x-auto rounded-lg shadow">
            <table class="w-full text-sm text-left border-collapse bg-white dark:bg-gray-800">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($customerGroups as $index => $customerGroup)
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-1">
                                        <div class="mb-2">
                                            ...
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $customerGroup->name ?? '-' }}</span>
                                        </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <x-crud.action-buttons :model="$customerGroup" resource="customer-groups" />
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p>No Customer Group available</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
