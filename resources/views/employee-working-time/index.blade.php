...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    ...
    <x-ui.card>
        <x-slot:header>
            <x-crud.page-header title="Employee Working Time" :createRoute="route('employee-working-times.create')" />
        </x-slot:header>
        ...
        <div class="overflow-x-auto rounded-lg shadow">
            <table class="w-full text-sm text-left border-collapse bg-white dark:bg-gray-800">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($employeeWorkingTimes as $index => $employeeWorkingTime)
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-1">
                                        <div class="mb-2">
                                            ...
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $employeeWorkingTime->employee_id ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Working Date:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $employeeWorkingTime->working_date ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Time Duration:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $employeeWorkingTime->time_duration ?? '-' }}</span>
                                        </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <x-crud.action-buttons :model="$employeeWorkingTime" resource="employee-working-times" />
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p>No Employee Working Time available</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
