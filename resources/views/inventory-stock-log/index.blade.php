                    ...
                    @forelse($inventoryStockLogs as $index => $inventoryStockLog)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 text-gray-900 dark:text-gray-100">
                                <strong>Employee Id:</strong> {{ $inventoryStockLog->employee_id ?? '-' }}<br><strong>Inventory Id:</strong> {{ $inventoryStockLog->inventory_id ?? '-' }}<br><strong>Type:</strong> {{ $inventoryStockLog->type ?? '-' }}<br><strong>Quantity:</strong> {{ $inventoryStockLog->quantity ?? '-' }}<br><strong>Cost:</strong> {{ $inventoryStockLog->cost ?? '-' }}
                            </td>
                            <td class="px-6 py-4">
