...
@section('content')
<x-worker.page 
    title="สรุปข้อมูลการซัก" 
    subtitle="Wash Operation Summary" 
...
>
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <x-worker.card title="สถานะการซัก">
            <div class="space-y-6">
                {{-- Action Buttons --}}
                <div class="grid grid-cols-3 gap-3">
                    <button type="button" 
                            onclick="window.location='{{ route('worker.operation.wash.select-linen-case', ['operationId' => $operation['id'], 'operationLinenProductId' => 0]) }}'"
                    ...
                    <button type="button" 
                            onclick="window.location='{{ route('worker.operation.wash.select-operation-linen-product', ['operationId' => $operation['id']]) }}'"
                ...
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-3">
                    ...
                    <div class="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-2">
                        <span class="text-gray-500 text-sm">น้ำหนักซักทั้งหมด</span>
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $operation['total_wet_weight'] }} kg.</span>
                    </div>
                    <div class="flex justify-between items-center pb-2">
                        <span class="text-gray-500 text-sm">เวลาในการซักผ้า</span>
                        ...
                        <div class="flex items-start p-3 bg-white dark:bg-gray-700 rounded-md border border-gray-100 dark:border-gray-600 shadow-sm">
                            ...
                            <div class="text-right ml-4">
                                <span class="block text-sm font-semibold text-gray-900 dark:text-white">
                                    {{ $operationLinenProduct['wet_weight'] ? $operationLinenProduct['wet_weight'] : '-' }} kg.
                                </span>
        ...
        <x-worker.card title="สรุปข้อมูลการซักของพนักงาน">
            <div class="flex flex-col items-center py-6 border-b border-gray-100 dark:border-gray-700">
                <div class="w-24 h-24 mb-4">
                    <x-employee-avatar :photo="$operation['wash_employee']['photo']" class="w-full h-full rounded-full shadow-md" />
                </div>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $operation['wash_employee']['name'] }}</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $operation['wash_employee']['code'] }}</p>
            </div>
    ...
    $(function(){
        ...
        function closeOperation(){
            removeOnUnload(function(){
                window.location='{{ route('worker.operation.wash.set-close', ['operationId' => $operation['id']]) }}'
            })
        }

        function reopenOperation(){
            removeOnUnload(function(){
                window.location='{{ route('worker.operation.wash.set-in-progress', ['operationId' => $operation['id']]) }}'
            })
        }
