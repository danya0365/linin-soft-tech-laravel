...
@section('content')
<x-worker.page 
    title="จำนวนที่พับแพ็คและสี" 
    subtitle="Packing Quantity and Color" 
    ...
    :breadcrumbs="[
        ...
        ['label' => 'ประเภทผ้า: ' . $operationLinenProduct['linen_product']['name'], 'route' => route('worker.operation.packing.select-linen-product', ['operationId' => $operation['id'], 'operationLinenProductId' => $operationLinenProduct['id']])],
        ['label' => 'จำนวน/สี']
    ]"
>
    <x-worker.card title="บันทึกข้อมูล">
        <form method="POST" action="{{ route('worker.operation.packing.set-select-weight-and-color', ['operationId' => $operation['id'], 'operationLinenProductId' => $operationLinenProduct['id']]) }}" role="form" enctype="multipart/form-data" class="space-y-6">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">จำนวน (ชิ้น)</h3>
                    <x-number-pad :inputName="'packing_piece'" :inputValue="$operationLinenProduct['packing_piece']">
                        จำนวนชิ้น
                    </x-number-pad>
                </div>
                <div>
