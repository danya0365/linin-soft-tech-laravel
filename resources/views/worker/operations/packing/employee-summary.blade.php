...
@section('content')
<x-worker.page 
    title="สรุปข้อมูลการพับแพ็ค" 
    subtitle="Packing Summary" 
...
>
    <div class="space-y-6">
        <!-- Status and Actions -->
        <x-worker.card title="สถานะการพับแพ็ค">
            <x-slot:action>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <button type="button" 
                        onclick="window.location='{{ route('worker.operation.packing.select-linen-case', ['operationId' => $operation['id'], 'operationLinenProductId' => 0]) }}'"
                ...
                <button type="button" 
                        onclick="window.location='{{ route('worker.operation.packing.select-operation-linen-product', ['operationId' => $operation['id']]) }}'"
            ...
            <div class="border-t border-gray-100 dark:border-gray-700 pt-4">
                <div class="flex justify-between items-center mb-4">
                    <span class="text-sm text-gray-500 dark:text-gray-400">จำนวนที่พับแพ็คทั้งหมด</span>
                    <span class="text-2xl font-bold text-gray-900 dark:text-white">
                        <i class="fa-solid fa-shirt text-gray-400 mr-2"></i>
                        {{ $operation['total_packing_piece'] ? number_format($operation['total_packing_piece']) : '0' }}
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-start justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg text-sm">
                        ...
                        <div class="text-right space-y-1">
                            <div class="font-medium text-gray-900 dark:text-white">
                                {{ $operationLinenProduct['packing_piece'] ? $operationLinenProduct['packing_piece'] : '0' }} ชิ้น
                            </div>
        ...
        <x-worker.card title="สรุปข้อมูลพนักงาน">
            <div class="flex flex-col items-center mb-6">
                <x-employee-avatar :photo="$operation['packing_employee']['photo']" class="w-24 h-24 rounded-full mb-3 shadow-md" />
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $operation['packing_employee']['name'] }}</h3>
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ $operation['packing_employee']['code'] }}</span>
            </div>
    ...
    $(function(){
        ...
        function closeOperation(){
            removeOnUnload(function(){
                window.location='{{ route('worker.operation.packing.set-close', ['operationId' => $operation['id']]) }}'
            })
        }

        function reopenOperation(){
            removeOnUnload(function(){
                window.location='{{ route('worker.operation.packing.set-in-progress', ['operationId' => $operation['id']]) }}'
            })
        }
