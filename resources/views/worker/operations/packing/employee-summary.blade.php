...
<div class="container">
    ...
    <div class="row justify-content-center">
        <div class="col-md-12 m-2">
            <div class="card">
                <div class="card-header text-center">สถานะการพับแพ็ค</div>
                <div class="card-body">
                    <div class="row g-2">
                        <div class="col-4">
                            <div class="d-grid gap-2" style="min-height: 60px">
                                <button class="btn btn-primary" type="button" onclick="window.location='{{ route('worker.operation.packing.select-linen-case', ['operationId' => $operation['id'], 'operationLinenProductId' => 0]) }}'">เพิ่ม</button>
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="d-grid gap-2" style="min-height: 60px">
                                <button class="btn btn-danger" type="button" onclick="window.location='{{ route('worker.operation.packing.select-operation-linen-product', ['operationId' => $operation['id']]) }}'">แก้ไขหรือลบ</button>
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="d-grid gap-2" style="min-height: 60px">
                                @if ( $operation['status'] != "close")
                                <button class="btn btn-outline-secondary" type="button" onclick="window.location='{{ route('worker.operation.packing.set-close', ['operationId' => $operation['id']]) }}'">ปิดงาน</button>
                                @else 
                                <button class="btn btn-outline-secondary" type="button" onclick="window.location='{{ route('worker.operation.packing.set-in-progress', ['operationId' => $operation['id']]) }}'">เปิดใหม่</button>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item">จำนวนที่พับแพ็คทั้งหมด: {{ $operation['total_packing_piece'] ? $operation['total_packing_piece'] : '-' }} ชิ้น</li>
                    ...
                    <li class="list-group-item">
                        ...
                        จำนวนที่พับแพ็ค: {{ $operationLinenProduct['packing_piece'] ? $operationLinenProduct['packing_piece'] : 'ยังไม่ได้เลือก' }} ชิ้น,
                ...
                <div class="card-footer text-muted text-center">
                    เวลาในการพับแพ็คผ้า: {{ $operationTimeDuration }}
                </div>
              </div>
        </div>
        <div class="col-md-12 m-2">
            <div class="card">
                <div class="card-header text-center">สรุปข้อมูลการพับแพ็คของพนักงาน</div>
                <div class="card-body">
                    ...
                    <dl class="row">
                        ...
                        <dd class="col-sm-9">{{ $operation['packing_employee']['code'] }}</dd>
                    </dl>
                </div>
                <ul class="list-group list-group-flush">
