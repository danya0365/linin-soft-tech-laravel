...
@section('content')
<x-worker.page 
    title="น้ำหนักอบและสี" 
    subtitle="Dry Weight and Color" 
    ...
    :breadcrumbs="[
        ...
        ['label' => 'เคสงาน: ' . $operationLinenProduct['linen_case']['name'], 'route' => route('worker.operation.dry.select-linen-case', ['operationId' => $operation['id'], 'operationLinenProductId' => $operationLinenProduct['id']])],
        ['label' => 'ประเภทผ้า: ' . $operationLinenProduct['linen_product']['name'], 'route' => route('worker.operation.dry.select-linen-product', ['operationId' => $operation['id'], 'operationLinenProductId' => $operationLinenProduct['id']])],
        ['label' => 'น้ำหนัก/สี']
    ]"
>
    <x-worker.card title="บันทึกข้อมูล">
        <form method="POST" action="{{ route('worker.operation.dry.set-select-weight-and-color', ['operationId' => $operation['id'], 'operationLinenProductId' => $operationLinenProduct['id']]) }}" role="form" enctype="multipart/form-data" class="space-y-6">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">น้ำหนัก (กิโลกรัม)</h3>
                    <x-number-pad :inputName="'dry_weight'" :inputValue="$operationLinenProduct['dry_weight']">
                        น้ำหนักกิโลกรัม
                    </x-number-pad>
                </div>
                <div>
