...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    <x-crud.breadcrumb :items="[['label' => 'Admin', 'route' => route('admin')],['label' => 'Department']]" />
    <x-ui.card>
        <x-slot:header>
                    ...
                    @forelse($departments as $index => $department)
                        <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent dark:hover:from-gray-700/50 dark:hover:to-transparent transition-all duration-200">
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-1">
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Var Name:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $department->var_name ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                        ...
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Input Unit:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $department->input_unit ?? '-' }}</span>
                                        </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <x-crud.action-buttons :model="$department" resource="departments" />
                            </td>
                        </tr>
                    @empty
                        <tr>
