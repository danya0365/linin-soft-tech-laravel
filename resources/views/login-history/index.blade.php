        ...
        <div class="overflow-x-auto rounded-lg shadow">
            <table class="w-full text-sm text-left border-collapse bg-white dark:bg-gray-800">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($loginHistories as $index => $loginHistory)
                        <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent dark:hover:from-gray-700/50 dark:hover:to-transparent transition-all duration-200">
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-1">
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">User Id:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $loginHistory->user_id ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Name:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $loginHistory->name ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Email:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $loginHistory->email ?? '-' }}</span>
                                        </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <x-crud.action-buttons :model="$loginHistory" resource="login-histories" />
                            </td>
                        </tr>
                    @empty
                        <tr>
