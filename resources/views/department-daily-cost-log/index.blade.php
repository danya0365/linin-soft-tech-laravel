        ...
        <div class="overflow-x-auto rounded-lg shadow">
            <table class="w-full text-sm text-left border-collapse bg-white dark:bg-gray-800">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($departmentDailyCostLogs as $index => $departmentDailyCostLog)
                        <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent dark:hover:from-gray-700/50 dark:hover:to-transparent transition-all duration-200">
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-1">
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Department Id:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $departmentDailyCostLog->department_id ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Daily Date:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $departmentDailyCostLog->daily_date ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Cost:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $departmentDailyCostLog->cost ?? '-' }}</span>
                                        </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <x-crud.action-buttons :model="$departmentDailyCostLog" resource="department-daily-cost-logs" />
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p>No Department Daily Cost Log available</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
