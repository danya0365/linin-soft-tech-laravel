        ...
        <div class="overflow-x-auto rounded-lg shadow">
            <table class="w-full text-sm text-left border-collapse bg-white dark:bg-gray-800">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($employeeOperationLogs as $index => $employeeOperationLog)
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-1">
                                        <div class="mb-2">
                                            ...
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $employeeOperationLog->employee_id ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Operation Type:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $employeeOperationLog->operation_type ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Action Type:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $employeeOperationLog->action_type ?? '-' }}</span>
                                        </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <x-crud.action-buttons :model="$employeeOperationLog" resource="employee-operation-logs" />
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p>No Employee Operation Log available</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
