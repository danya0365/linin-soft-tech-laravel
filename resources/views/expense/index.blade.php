                    ...
                    @forelse($expenses as $index => $expense)
                        <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent dark:hover:from-gray-700/50 dark:hover:to-transparent transition-all duration-200">
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-1">
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Type Name:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $expense->type_name ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Table Name:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $expense->table_name ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Table Id:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $expense->table_id ?? '-' }}</span>
                                        </div>
                                        <div class="mb-2">
                                            <span class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Amount:</span>
                                            <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ $expense->amount ?? '-' }}</span>
                                        </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <x-crud.action-buttons :model="$expense" resource="expenses" />
                            </td>
                        </tr>
                    @empty
                        <tr>
